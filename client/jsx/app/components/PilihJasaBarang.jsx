...
const PilihJasaBarang = ({ barangList, setBarangList, servicesAndStock, errors }) => {
    const [selectedBarang, setSelectedBarang] = useState(null);
    const [filteredItems, setFilteredItems] = useState(servicesAndStock);
    const [itemFilter, setItemFilter] = useState('');
    useEffect(() => {
        setFilteredItems(servicesAndStock.filter(item => item.KETERANGAN?.toLowerCase().includes(itemFilter.toLowerCase()) ||
    ...
    const updateBarang = useCallback((index, field, value) => {
        setBarangList(prev => {
            const newList = [...prev];
    ...
    const statusTemplate = (rowData) => (<Dropdown value={rowData.STATUSAMBIL} options={['Antrian', 'Proses', 'Selesai']} onChange={(e) => {
            e.stopPropagation();
            updateBarang(barangList.findIndex(b => b.KODE === rowData.KODE), 'STATUSAMBIL', e.value);
        }} placeholder="Select Status" onClick={(e) => e.stopPropagation()}/>);
    const estimatedPriceTemplate = (rowData) => (<span>{formatCurrency(rowData.ESTIMASIHARGA)}</span>);
    const actionTemplate = (rowData) => (<div className="flex gap-2">
    ...
    return (<div className="grid">
            <div className="col-12">
                ...
                <DataTable value={barangList} selection={selectedBarang} onSelectionChange={(e) => setSelectedBarang(e.value)} selectionMode="single" dataKey="KODE">
            ...
            {selectedBarang && (<>
                    ...
                    <div className="col-12 md:col-6 mt-3">
                        ...
                        <DataTable value={groupAndSumServices(selectedBarang?.services)}>
                            ...
                            <Column body={(rowData) => (<div className="flex items-center">
                                    <Button icon="pi pi-minus" className="p-button-rounded p-button-danger p-button p-component p-button-icon-only" onClick={() => removeItemFromBarang(rowData.KODE)}/>
                                </div>)}></Column>
                        </DataTable>
                    </div>
                </>)}
