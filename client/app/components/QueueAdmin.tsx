...
const QueueAdmin: React.FC = () => {
    ...
    const recall = async (queue: Queue) => {
        try {
    ...
    const finish = async (queue: Queue) => {
        try {
    ...
    const updateQueueNumber = async (queue: Queue, newNumber: number) => {
        try {
        ...
        <div>
            <div className="grid mb-4">
                ...
                {queues
                    ...
                    .map((queue) => (
                        <div key={queue.id} className={`col-12 md:col-6 lg:col-3 ${active?.id === queue.id ? 'shadow-2' : ''}`}>
                            <div className="surface-card p-4 shadow-2 border-round">
                                <h2>{queue.name}</h2>
                                <InputNumber
                                    value={queue.number}
                                    onValueChange={(e) => updateQueueNumber(queue, e.value as number)}
